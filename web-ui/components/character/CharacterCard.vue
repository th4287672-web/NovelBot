<template>
  ...
  >
    ...
    <div v-if="character.is_private" class="p-3 border-t border-gray-600/80 flex justify-end space-x-2">
      <button @click.stop="emit('share', character)" class="btn btn-secondary !px-3 !py-1 text-xs bg-indigo-600/20 hover:bg-indigo-500/30 border-indigo-500/50 text-indigo-300">分享</button>
      <button @click.stop="emit('edit', character)" class="btn btn-secondary !px-3 !py-1 text-xs">编辑</button>
