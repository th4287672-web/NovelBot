<template>
  <div
    class="bg-gray-800/60 p-3 rounded-sm flex items-center justify-between border border-gray-600/50 transition-all group"
  ...
  >
    ...
    <div class="flex items-center space-x-2 ml-4 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
      <button v-if="character.is_private" @click.stop="emit('share', character)" class="btn btn-secondary !px-2 !py-1 text-xs bg-indigo-600/20 hover:bg-indigo-500/30 border-indigo-500/50 text-indigo-300" title="分享">🔗</button>
      <button v-if="character.is_private" @click.stop="emit('edit', character)" class="btn btn-secondary !px-2 !py-1 text-xs" title="编辑">✏️</button>
